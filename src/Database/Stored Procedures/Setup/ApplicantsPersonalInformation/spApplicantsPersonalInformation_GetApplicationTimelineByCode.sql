...
AS
 ...
 --       apl.Code = @code
	...
	SELECT DISTINCT
		apl.Code,
		0 AS ApprovalStatusNumber,
		'Application In Draft' AS ApplicationStatus,
		'Credit Verification' AS Stage,
		1 AS StageNo,
		0 AS ApproverRoleId,
		apl.DateCreated
	FROM
	...
	SELECT
		apl.Code,
		appLog.[Action] AS ApprovalStatusNumber,
		CASE
			WHEN appLog.[Action] = 0 THEN 'Application in Draft'
			WHEN appLog.[Action] = 1 THEN 'Application Submitted'
			WHEN appLog.[Action] = 3 THEN 'Verified by Developer'
	...
	WHERE
		...
		AND NOT EXISTS (
			SELECT 1
			FROM ApprovalLog
			WHERE ReferenceId = apl.Id
				AND [Action] = 1
				AND DateCreated > appLog.DateCreated
		);
RETURN 0