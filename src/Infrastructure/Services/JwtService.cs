...
public class JwtService : IJwtService
{
    ...
    public async Task<User> Authenticate(string token)
    {
        ...
        if (!isValidToken)
        {
            ...
            await _authenticationService.InsertUserActivity(user.Id, "Failed token Login");
            throw new Exception("Invalid token");
        }

        await _authenticationService.InsertUserActivity(user.Id, "Successful token Login");
        return user;
    }
