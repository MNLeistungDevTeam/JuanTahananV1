...
$(function () {
    ...
    if (companyId != 0) {
        tbl_company = $("#tbl_company").DataTable({
            ajax: {
                url: baseUrl + "Company/GetSubCompanies",
                data: function (d) {
                    d.id = companyId;
                },
                dataSrc: ""
            ...
            columns: [
                {
                    ...
                    render: function (data, type, row) {
                        return `${data ?? 'N/A'} <p class="text-muted m-0">${row.Tin ?? 'N/A'}</p><p class="text-muted m-0">${row.BusinessStyle ?? 'N/A'}</p>`
                    }
                }
            ],
        ...
        tbl_company.on('select', function () {
            var all = tbl_company.rows({ search: 'applied' }).count();
            var selectedRows = tbl_company.rows({ selected: true, search: 'applied' }).count();
            var subcompanyId = tbl_company.rows({ selected: true }).data().pluck("Id").toArray().toString();

            /* Id = Id == "" ? companyId : Id;*/

            loadCompanyProfile(subcompanyId);
        });

        $("#btn_reset_company_select").on("click", function () {
            tbl_company.data().rows().deselect();
    ...
    function resetForm() {
        ...
        //$("select[name='Setting.AccountingPeriod']")[0].selectize.setValue("");
        //$("select[name='Setting.InvEvalMethodId']")[0].selectize.setValue(0);
        //$("select[name='Setting.Bir2307Basis']")[0].selectize.setValue(0);

        if (companyId != 0) {
            //$("select[name='Setting.AccountingPeriod']")[0].selectize.setValue("Calendar");
            //$("select[name='Setting.InvEvalMethodId']")[0].selectize.setValue(1);
        }
        ...
        //document.getElementById("info-tab-link").click();
    }
